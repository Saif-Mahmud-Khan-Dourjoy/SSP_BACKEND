<?php
	namespace App\Http\Controllers\Api\Aggregate;

	use App\Http\Controllers\Controller;
	use DB;

	class Sufal192020_planting_plan_gplanting_gspp extends Controller
	{
		
		public function sufal192020_planting_plan_gplanting_gspp_create(){
			$data = array(
				"_URI" => Request::input("_uri"),
"_CREATOR_URI_USER" => Request::input("_creator_uri_user"),
"_CREATION_DATE" => Request::input("_creation_date"),
"_LAST_UPDATE_URI_USER" => Request::input("_last_update_uri_user"),
"_LAST_UPDATE_DATE" => Request::input("_last_update_date"),
"_PARENT_AURI" => Request::input("_parent_auri"),
"_ORDINAL_NUMBER" => Request::input("_ordinal_number"),
"_TOP_LEVEL_AURI" => Request::input("_top_level_auri"),
"PREF_SOURCE" => Request::input("pref_source"),
"PREF_OTHER_SOURCE" => Request::input("pref_other_source"),
"PREF_SPECIES" => Request::input("pref_species"),
"PREF_REPRO_TYPE" => Request::input("pref_repro_type"),
"PREF_NRSEEDLINGS" => Request::input("pref_nrseedlings"),

			);
			$insert_id = DB::table("SUFAL192020_PLANTING_PLAN_GPLANTING_GSPP")->insertGetId($data);
			return response()->json($insert_id);
		}
		public function sufal192020_planting_plan_gplanting_gspp_list(){
			$query = DB::select('select * from aggregate."SUFAL192020_PLANTING_PLAN_GPLANTING_GSPP"');
			return response()->json($query);
		}
	}
			
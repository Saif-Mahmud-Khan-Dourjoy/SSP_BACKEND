...
class Com202021_core extends Controller
{
	...
	public function com202021_core_create(Request $request)
	{
		...
		$response = DB::table("aggregate.COM202021_CORE")->insert($data);
		$q = DB::GetQueryLog();
	...
	public function com202021_core_list_by_created_by($createdBy)
	{
		$query = DB::table("aggregate.COM202021_CORE")->where('created_by', $createdBy)->get();
		return response()->json($query);
	}

	public function com202021_core_uri($uri)
	{
		$query = DB::table("aggregate.COM202021_CORE")->where('_URI', $uri)->first();
		return response()->json($query);
	}

	public function com202021_core_single_delete($uri)
	{
		$query = DB::table("aggregate.COM202021_CORE")->where('_URI', $uri)->delete();
		return response()->json($query);
	}


	public function com202021_core_update(Request $request, $uri)
	{
		...
		$response = DB::table("aggregate.COM202021_CORE")->where("_URI", $uri)->update($data);
		if ($response) {

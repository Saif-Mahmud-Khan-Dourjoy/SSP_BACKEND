...
use Illuminate\Http\Request;
	...
	class Plant27_2021_gregen_gregen_plot_reg_cen_to_s_blb extends Controller
	{

	// 		public function plant27_2021_gregen_gregen_plot_reg_cen_to_s_blb_create(){
	// 			$data = array(
	...
	// 			);
	// 			$insert_id = DB::table("PLANT27_2021_GREGEN_GREGEN_PLOT_REG_CEN_TO_S_BLB")->insertGetId($data);
	// 			return response()->json($insert_id);
	// 		}
	// 		public function plant27_2021_gregen_gregen_plot_reg_cen_to_s_blb_list(){
	// 			$query = DB::select('select * from aggregate."PLANT27_2021_GREGEN_GREGEN_PLOT_REG_CEN_TO_S_BLB"');
	// 			return response()->json($query);
	// 		}

	public function plant27_2021_gregen_gregen_plot_reg_cen_to_s_blb_create(Request $request)
	{
		$data = array(
			"_URI" => $request->input("_uri"),
			"_CREATOR_URI_USER" => $request->input("_creator_uri_user"),
		...
		$response = DB::table("aggregate.PLANT27_2021_GREGEN_GREGEN_PLOT_REG_CEN_TO_S_BLB")->insert($data);
	...
	public function plant27_2021_gregen_gregen_plot_reg_cen_to_s_blb_bulk_create(Request $request)
	{
		...
		foreach ($payload as $item) {
			$data[] = [
				...
				"VALUE" => isset($item["value"]) ? mb_convert_encoding($item['value'], 'UTF-8',
						'UTF-8'
					) : null,
			];
		}
	...
	public function plant27_2021_gregen_gregen_plot_reg_cen_to_s_blb_tlauri($tlauri)
	{
		...
		$sql = "
            select
                gen.*
                from 
                aggregate.\"PLANT27_2021_GREGEN_GREGEN_PLOT_REG_CEN_TO_S_BLB\" as gen
	...
	public function plant27_2021_gregen_gregen_plot_reg_cen_to_s_blb_update(Request $request, $auri)
	{
		...
		foreach ($payload as $item) {
			$data[] = [
				...
				"_LAST_UPDATE_DATE" => $item["_last_update_date"] ?? null,
				"_TOP_LEVEL_AURI" => $item["_top_level_auri"] ?? null,
				"VALUE" => isset($item["value"]) ? mb_convert_encoding(
					$item['value'],
					'UTF-8',
					'UTF-8'
				) : null,
